...
protocol  MomentImageBrowserDelegate : NSObjectProtocol {
    func getTheThumbnailImage(_ indexRow: Int) ->UIImage
}


class MomentImageBrower: UIView,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    
    var delegate :MomentImageBrowserDelegate?
    ...
    func  setupUI(){
        ...
        fowLayout.itemSize = CGSize(width: ScreenWidth + imageInterval,height: ScreenHeight)
        collectionView = UICollectionView.init(frame: CGRect(x: 0,
            y: 0,width: ScreenWidth + imageInterval, height: ScreenHeight),collectionViewLayout: fowLayout)
